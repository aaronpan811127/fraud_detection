...
The databricks CLI bundles top level is defined by file `fraud_detection/databricks.yml`.
...
ML Resource Configurations in this directory:
 - model workflow (`fraud_detection/resources/model-workflow-resource.yml`)
 - batch inference workflow (`fraud_detection/resources/batch-inference-workflow-resource.yml`)
 - monitoring resource and workflow (`fraud_detection/resources/monitoring-resource.yml`)
 - feature engineering workflow (`fraud_detection/resources/feature-engineering-workflow-resource.yml`)
 - model definition and experiment definition (`fraud_detection/resources/ml-artifacts-resource.yml`)
...
If you want to use different catalog names, please update the `targets` declared in the `fraud_detection/databricks.yml` and `fraud_detection/resources/ml-artifacts-resource.yml` files.
...
|-------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|----------------------|-------------------------------------|------------------------------------------------|
| dev         | The `dev` deployment target is used by ML engineers to deploy ML resources to development workspace with `dev` configs. The config is for ML project development purposes.                                                           | dev workspace        | dev-fraud_detection-model     | /dev-fraud_detection-experiment     |
| staging     | The `staging` deployment target is part of the CD pipeline. Latest main content will be deployed to staging workspace with `staging` config.                                                             | staging workspace    | staging-fraud_detection-model | /staging-fraud_detection-experiment |
| prod        | The `prod` deployment target is part of the CD pipeline. Latest release content will be deployed to prod workspace with `prod` config.                                                                      | prod workspace       | prod-fraud_detection-model    | /prod-fraud_detection-experiment    |
| test        | The `test` deployment target is part of the CI pipeline. For changes targeting the main branch, upon making a PR, an integration test will be triggered and ML resources deployed to the staging workspace defined under `test` deployment target. | staging workspace    | test-fraud_detection-model    | /test-fraud_detection-experiment    |
...
2. Complete the `TODO` in `fraud_detection/databricks.yml` to add the dev workspace URI under `targets.dev.workspace.host`.
...
```
Following this, update the batch_inference_job base parameters in `fraud_detection/resources/batch-inference-workflow-resource.yml` to pass
...
To get started, open `fraud_detection/resources/batch-inference-workflow-resource.yml`.  The file contains the ML resource definition of a batch inference job, like:
...
resources:
  jobs:
    batch_inference_job:
      name: ${bundle.target}-fraud_detection-batch-inference-job
      tasks:
...
The example above defines a Databricks job with name `${bundle.target}-fraud_detection-batch-inference-job`
that runs the notebook under `fraud_detection/deployment/batch_inference/notebooks/BatchInference.py` to regularly apply your ML model for batch inference. 
...
We specify a `batch_inference_job` under `resources/jobs` to define a databricks workflow with internal key `batch_inference_job` and job name `{bundle.target}-fraud_detection-batch-inference-job`.
...
`test-fraud_detection-batch-inference-job`. During the deployment of the `staging` environment config, the job name will be
`staging-fraud_detection-batch-inference-job`.
